...
function shell({ lang, kind, slug, content, type, image = '/og-image.png', extraSchema = {} }) {
  const isEn = lang === 'en';
  const home = isEn ? '/en/' : '/';
...
function renderBody(lang, kind, c, image, extra) {
  ...
  return `<main><section class="hero"><div><div class="eyebrow">WITER / ${c.label}</div><h1>${c.title}</h1></div><div class="hero__bottom"><p>${c.intro}</p><a class="round-link" href="#approach">${isEn?'Explore<br>approach ↓':'Дізнатися<br>більше ↓'}</a></div></section><section class="section" id="approach"><div class="section__head"><h2>${c.detailTitle}</h2><p>${c.detail}</p></div><div class="grid">${cards}</div></section><section class="cta"><h2>${isEn?'Let’s build it.':'Створімо це.'}</h2><a href="${isEn?'/en/':'/'}#contact">${isEn?'Start a<br>project ↗':'Обговорити<br>проєкт ↗'}</a></section></main>`;
}

function write(file, content) { fs.mkdirSync(path.dirname(file), { recursive:true }); fs.writeFileSync(file, content); }
for (const item of services) for (const lang of ['uk','en']) write(filePath(lang,'services',item.slug), shell({lang,kind:'services',slug:item.slug,content:item[lang],type:'Service'}));
