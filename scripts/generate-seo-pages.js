...
function renderBody(lang, kind, c, image, extra) {
  ...
  const faqHtml = faq.map(([question,answer],index)=>`<details class="faq-item reveal"${index===0?' open':''}><summary><span>0${index+1}</span><strong>${question}</strong><i>+</i></summary><p>${answer}</p></details>`).join('');
